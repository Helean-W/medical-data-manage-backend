    ...
    for item in list:
        ...
        if Position == '胰腺':
            ...
            info["PatientAge"] = re.sub(
                "\D", "", info["PatientAge"])  # 把字符串中的数字提取出来
        ...
        Patient.objects.create(
            gender=info['PatientSex'], age=info["PatientAge"], position=info['Position'], url=info['Url'])
...
def importSingle(info, path, fname):
    ...
    os.rename(path, name)
    Patient.objects.create(
        gender=info['gender'], age=info["age"], position=info['position'], url=info['url'])
    shutil.move(name, './resources')
